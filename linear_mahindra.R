...
train_data <- dataset_linear[which(dataset_linear$`dataset$is_train` == TRUE),]
train_data$`dataset$is_train` <- NULL
...
split_indices <- sample.split(train_data_indices, SplitRatio = 0.7)

train_data <- train_data_indices[split_indices,]
...
aml_final <- h2o.automl(y = 'amount_spent_per_room_night_scaled',
                  training_frame = as.h2o(train_data_indices),
                  max_models = 5,
                  sort_metric = "RMSE",
                  seed = 1)
...
write.csv(cbind(test$reservation_id, as.data.frame(pred_auto_ml_final)),
          'sub_final.csv',row.names = F)
